...
export default function CarCard({ car }: CarCardProps) {
  return (
    <Link href={`/cars/${car.id}`}>
      <div className="group neon-border backdrop-blur-custom rounded-lg overflow-hidden hover:shadow-lg sm:hover:shadow-2xl transition-all duration-300 sm:duration-500 cursor-pointer h-full flex flex-col hover:border-accent-secondary sm:hover:scale-105 transform">
        {car.images.length > 0 ? (
          <div className="relative w-full h-48 bg-gradient-to-br from-primary to-secondary overflow-hidden">
            <Image
              ...
              alt={car.title}
              fill
              className="object-cover sm:group-hover:scale-125 transition-transform duration-300 sm:duration-700 brightness-90 sm:group-hover:brightness-110"
              loading="lazy"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300" />
            {car.video && (
              <div className="absolute top-3 right-3 bg-red-600 text-white px-3 py-1 rounded text-xs font-bold opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
                🎥 VIDEO
              </div>
            )}
            <div className="absolute top-3 right-3 bg-accent text-primary px-3 py-1 rounded text-xs font-bold opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300">
              FEATURED
            </div>
        ...
        <div className="p-5 flex flex-col flex-grow bg-secondary/50">
          <h3 className="font-bold text-lg text-primary-foreground mb-3 line-clamp-2 sm:group-hover:text-accent transition-colors duration-300 glow-text">
            {car.title}
          </h3>
